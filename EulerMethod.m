%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%&&******……%%%%%%%%
%%     此程序是根据文章"A completely explicit finite element method for solving                       %%
%%     dynamic u-p equations of fluid-saturated porous media"中介绍的方法来编写，求解孔隙压力时没用欧拉矫正 %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%&&&&&&&&&&&&&&&&&77
...
1*(10^-4)/(k_darcy*density_solid)
30*(10^-4)/(k_darcy*density_solid)

%totlstep=3;
%%%k_darcy*density_solid/(10^-4)=14.942
% +++++++++++++++++++++++++++++
%            MESHING
% +++++++++++++++++++++++++++++
% ---------------------------------------
...
for i=1:number_element
  ...
  k=BilinearQuadElementStiffness(E,NU,h,x1,y1,x2,y2,x3,y3,x4,y4,2);
...
pressurenext=0.00*ones(totln,1);
%uw=zeros(2*totln,1); 
...
for tt=1:totlstep
 ...
 fprintf('time %f***开始逐步迭代计算***\r\n',time);
    ...
    vvs=invmass*(fu-K*us+Q*pressure); 
...
temp1=2*eye(2*totln,2*totln)-invmass*K*dt^2;
temp2=invmass*Q*dt^2;
temp3=invmass*dt^2;
usnext=temp1*us-us_before+temp2*pressure+temp3*fu;
...
%%%%%%%%%%%%%%%%%%%%位移边界条件
   ...
   %%%%%%%%%%%%%%%%%%%%%%孔压计算%%%%%%%%%%%%%%%%%%%%
   temp4=eye(totln,totln)-invdamp*J*dt;
   temp5=invdamp*Q';
   temp6=invdamp*dt;
   pressurenext=temp4*pressure+temp5*(-us+us_before)+temp6*Fp;
...
%%%%%%%%%%孔压边界条件%%%%%%%%%% 
 ...
 %                  输出信息                    %
  ...
  %  fprintf(fid1,'%f %f\r\n',time/(0.004883*density_solid),us(402)); 
    fprintf(fid1,'%f %f\r\n',time/(0.004883*density_solid),usnext(402)*Vc/k_darcy); %获得顶点在0到tao的位移变化
    fprintf(fid2,'%f %f\r\n',time/(0.004883*density_solid),pressurenext(193)); %见Simon的文章，求解结果为
  end
...
% scus4=scus(:,4); 
 figure (1)
 plot(time,scus2,'-*g','linewidth', 1.5)
 hold on
...
% scpp4=scpp(:,4); 
% 
 plot(time1,scpp2,'-ok','linewidth', 1.5)
 hold on
% plot(time1,scpp3,'g','linewidth', 1.5)
% hold on
% plot(time1,scpp4,'g','linewidth', 1.5)
% hold on
gca=legend({'Xu^\primes Method'},'location','SouthEast','FontSize',14);
