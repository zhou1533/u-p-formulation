...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%      此程序是根据宋佳文章"流体饱和多孔介质动力问题的显式时域解法" %%
%%           中介绍的方法来编写，文章中运用了欧拉矫正             %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
temp1=eye(2*totln,2*totln)-0.5*invmass*K*dt^2;
temp2=dt*vs;
temp3=0.5*invmass*Q*dt^2;
temp4=0.5*invmass*dt^2;
usnext=temp1*us+temp2+temp3*pressure+temp4*fu;
...
%%%%%%%%%%位移边界条件%%%%%%%%%%%%%%%%%%%%%%
  ...
  vsnext=kama/(beta*dt)*(usnext-us)+(1-kama/beta)*vs;
   ...
   pressurenext0=pressure-(invdamp*J*pressure-invdamp*Q'*vs+invdamp*Fp)*dt;
   
   temp5=0.5*invdamp*J;
   temp6=0.5*invdamp*Q';
   temp7=0.5*invdamp;
   pressurenext=pressure-temp5*(pressure+pressurenext0)*dt-temp6*(vs+vsnext)*dt+temp7*(Fp+Fp)*dt;
...
% scus4=scus(:,4); 
 figure (2)
 plot(time,scus2,'-ok','linewidth', 1.5)
 hold on
...
% scpp4=scpp(:,4); 
% 
 plot(time1,scpp2,'-*g','linewidth', 1.5)
 gca=legend({'Proposed Method'},'location','SouthEast','FontSize',14);
